...
try{
  ...
  ***********/
  ...
  $dbquery = $db->query($statment . " " . $WHEREclause . " ORDER BY ProductsID ASC " . " Limit " . $limitNumber);
  ...
  for ($i=0; $i < count($results); $i++) {
    ...
    foreach ($results[$i] as $key => $value) {
      ...
      if($key == "rarityname"){
        ...
        //Button to remove 1 product from the table onclick=\"addToCart(" . $results[$i] .")\">"
        $button = "<td><button type=\"button\" value=\"$value\" onclick=\"addToCart(" . $value .", &quot;SearchForm&quot;, &quot;productTable&quot;)\">"  . ($i+1) . "</button></td>";
        $returnString .= $button;
      }
      else {
